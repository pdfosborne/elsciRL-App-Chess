...
# Engine cannot use relative imports if called by elsciRL GUI so all functions must be defined here
class RandomAgentFiltered(Agent):
    ...
    def policy(self, board_fen:str, legal_moves:List[str]=None):
        # Go through each known move from player data and extract move_uci of known moves
        if board_fen in self.player_data_dict:
            filtered_legal_moves = []
            for move in self.player_data_dict[board_fen]:
                if self.player_data_dict[board_fen][move]["totalGames"] > 10:
